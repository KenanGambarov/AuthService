...
import static com.authservice.exception.ExceptionConstants.INVALID_TOKEN;
import static com.authservice.exception.ExceptionConstants.USER_NOT_FOUND;
...
public class GlobalExceptionHandler {
    ...
    public ExceptionResponse handleNotFoundException(UnAuthorizedException ex) {
        log.warn("UnAuthorizedException: {}", ex.getMessage());

        return ExceptionResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.UNAUTHORIZED.value())
                .errors(INVALID_TOKEN.getMessage())
                .message(ex.getMessage())
                .build();
    }
